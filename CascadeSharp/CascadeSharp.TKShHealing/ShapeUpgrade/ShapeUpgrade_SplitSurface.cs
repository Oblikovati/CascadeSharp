//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKernel;
using CascadeSharp.TKernel.TColStd;
using CascadeSharp.TKG3d.Geom;
using CascadeSharp.TKShHealing.ShapeExtend;

namespace CascadeSharp.TKShHealing.ShapeUpgrade
{
	//---------------------------------------------------------------------
	//  Class  ShapeUpgrade_SplitSurface
	//---------------------------------------------------------------------
	public  class ShapeUpgrade_SplitSurface : Standard_Transient
	{

		public ShapeUpgrade_SplitSurface()
			: base()
		{
			throw new NotImplementedException();
		}

		public ShapeUpgrade_SplitSurface(ShapeUpgrade_SplitSurface parameter1)
			: base()
		{
			throw new NotImplementedException();
		}

		public void Init(Geom_Surface S)
		{
			throw new NotImplementedException();
		}

		public void Init(Geom_Surface S, double UFirst, double ULast, double VFirst, double VLast)
		{
			throw new NotImplementedException();
		}

		public void SetUSplitValues(TColStd_HSequenceOfReal UValues)
		{
			throw new NotImplementedException();
		}

		public void SetVSplitValues(TColStd_HSequenceOfReal VValues)
		{
			throw new NotImplementedException();
		}

		public void Build(bool Segment)
		{
			throw new NotImplementedException();
		}

		public void Compute(bool Segment)
		{
			throw new NotImplementedException();
		}

		public void Compute()
		{
			throw new NotImplementedException();
		}

		public void Perform(bool Segment)
		{
			throw new NotImplementedException();
		}

		public void Perform()
		{
			throw new NotImplementedException();
		}

		public TColStd_HSequenceOfReal USplitValues()
		{
			throw new NotImplementedException();
		}

		public TColStd_HSequenceOfReal VSplitValues()
		{
			throw new NotImplementedException();
		}

		public bool Status(ShapeExtend_Status status)
		{
			throw new NotImplementedException();
		}

		public ShapeExtend_CompositeSurface ResSurfaces()
		{
			throw new NotImplementedException();
		}

		public char get_type_name()
		{
			throw new NotImplementedException("Native class returns pointer to integer/double/handle.");
		}

				
	}; // class ShapeUpgrade_SplitSurface

}
