//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKernel;
using CascadeSharp.TKG3d.Geom;

namespace CascadeSharp.TKG3d.TColGeom
{
	//---------------------------------------------------------------------
	//  Class  TColGeom_HArray1OfBSplineCurve
	//---------------------------------------------------------------------
	public  sealed class TColGeom_HArray1OfBSplineCurve : Standard_Transient
	{

		public TColGeom_HArray1OfBSplineCurve()
			: base()
		{
			throw new NotImplementedException();
		}

		public TColGeom_HArray1OfBSplineCurve(int theLower, int theUpper)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColGeom_HArray1OfBSplineCurve(int theLower, int theUpper, Geom_BSplineCurve theValue)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColGeom_HArray1OfBSplineCurve(TColGeom_Array1OfBSplineCurve theOther)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColGeom_HArray1OfBSplineCurve(TColGeom_HArray1OfBSplineCurve parameter1)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColGeom_Array1OfBSplineCurve Array1()
		{
			throw new NotImplementedException();
		}

		public TColGeom_Array1OfBSplineCurve ChangeArray1()
		{
			throw new NotImplementedException();
		}

		public char get_type_name()
		{
			throw new NotImplementedException("Native class returns pointer to integer/double/handle.");
		}

														public void Init(Geom_BSplineCurve theValue)
		{
			throw new NotImplementedException();
		}

		public int Size()
		{
			throw new NotImplementedException();
		}

		public int Length()
		{
			throw new NotImplementedException();
		}

		public bool IsEmpty()
		{
			throw new NotImplementedException();
		}

		public int Lower()
		{
			throw new NotImplementedException();
		}

		public int Upper()
		{
			throw new NotImplementedException();
		}

		public bool IsDeletable()
		{
			throw new NotImplementedException();
		}

		public bool IsAllocated()
		{
			throw new NotImplementedException();
		}

						public Geom_BSplineCurve First()
		{
			throw new NotImplementedException();
		}

		public Geom_BSplineCurve ChangeFirst()
		{
			throw new NotImplementedException();
		}

		public Geom_BSplineCurve Last()
		{
			throw new NotImplementedException();
		}

		public Geom_BSplineCurve ChangeLast()
		{
			throw new NotImplementedException();
		}

		public Geom_BSplineCurve Value(int theIndex)
		{
			throw new NotImplementedException();
		}

		public Geom_BSplineCurve ChangeValue(int theIndex)
		{
			throw new NotImplementedException();
		}

		public void SetValue(int theIndex, Geom_BSplineCurve theItem)
		{
			throw new NotImplementedException();
		}

		public void Resize(int theLower, int theUpper, bool theToCopyData)
		{
			throw new NotImplementedException();
		}


	}; // class TColGeom_HArray1OfBSplineCurve

}
