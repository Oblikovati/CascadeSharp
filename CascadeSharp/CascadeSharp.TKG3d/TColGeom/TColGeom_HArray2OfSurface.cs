//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKernel;
using CascadeSharp.TKG3d.Geom;

namespace CascadeSharp.TKG3d.TColGeom
{
	//---------------------------------------------------------------------
	//  Class  TColGeom_HArray2OfSurface
	//---------------------------------------------------------------------
	public  sealed class TColGeom_HArray2OfSurface : Standard_Transient
	{

		public TColGeom_HArray2OfSurface(int theRowLow, int theRowUpp, int theColLow, int theColUpp)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColGeom_HArray2OfSurface(int theRowLow, int theRowUpp, int theColLow, int theColUpp, Geom_Surface theValue)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColGeom_HArray2OfSurface(TColGeom_Array2OfSurface theOther)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColGeom_HArray2OfSurface(TColGeom_HArray2OfSurface parameter1)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColGeom_HArray2OfSurface()
		{
			throw new NotImplementedException();
		}

		public TColGeom_Array2OfSurface Array2()
		{
			throw new NotImplementedException();
		}

		public TColGeom_Array2OfSurface ChangeArray2()
		{
			throw new NotImplementedException();
		}

		public char get_type_name()
		{
			throw new NotImplementedException("Native class returns pointer to integer/double/handle.");
		}

						public void Init(Geom_Surface theValue)
		{
			throw new NotImplementedException();
		}

		public int Size()
		{
			throw new NotImplementedException();
		}

		public int Length()
		{
			throw new NotImplementedException();
		}

		public int NbRows()
		{
			throw new NotImplementedException();
		}

		public int NbColumns()
		{
			throw new NotImplementedException();
		}

		public int RowLength()
		{
			throw new NotImplementedException();
		}

		public int ColLength()
		{
			throw new NotImplementedException();
		}

		public int LowerRow()
		{
			throw new NotImplementedException();
		}

		public int UpperRow()
		{
			throw new NotImplementedException();
		}

		public int LowerCol()
		{
			throw new NotImplementedException();
		}

		public int UpperCol()
		{
			throw new NotImplementedException();
		}

		public bool IsDeletable()
		{
			throw new NotImplementedException();
		}

						public Geom_Surface Value(int theRow, int theCol)
		{
			throw new NotImplementedException();
		}

		public Geom_Surface ChangeValue(int theRow, int theCol)
		{
			throw new NotImplementedException();
		}

		public void SetValue(int theRow, int theCol, Geom_Surface theItem)
		{
			throw new NotImplementedException();
		}

		public void Resize(int theRowLower, int theRowUpper, int theColLower, int theColUpper, bool theToCopyData)
		{
			throw new NotImplementedException();
		}

		public void Allocate()
		{
			throw new NotImplementedException();
		}

		public void fillIndexTable(Geom_Surface theTable)
		{
			throw new NotImplementedException();
		}


	}; // class TColGeom_HArray2OfSurface

}
