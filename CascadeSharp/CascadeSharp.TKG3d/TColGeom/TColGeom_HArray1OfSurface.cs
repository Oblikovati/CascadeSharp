//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKernel;
using CascadeSharp.TKG3d.Geom;

namespace CascadeSharp.TKG3d.TColGeom
{
	//---------------------------------------------------------------------
	//  Class  TColGeom_HArray1OfSurface
	//---------------------------------------------------------------------
	public  sealed class TColGeom_HArray1OfSurface : Standard_Transient
	{

		public TColGeom_HArray1OfSurface()
			: base()
		{
			throw new NotImplementedException();
		}

		public TColGeom_HArray1OfSurface(int theLower, int theUpper)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColGeom_HArray1OfSurface(int theLower, int theUpper, Geom_Surface theValue)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColGeom_HArray1OfSurface(TColGeom_Array1OfSurface theOther)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColGeom_HArray1OfSurface(TColGeom_HArray1OfSurface parameter1)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColGeom_Array1OfSurface Array1()
		{
			throw new NotImplementedException();
		}

		public TColGeom_Array1OfSurface ChangeArray1()
		{
			throw new NotImplementedException();
		}

		public char get_type_name()
		{
			throw new NotImplementedException("Native class returns pointer to integer/double/handle.");
		}

														public void Init(Geom_Surface theValue)
		{
			throw new NotImplementedException();
		}

		public int Size()
		{
			throw new NotImplementedException();
		}

		public int Length()
		{
			throw new NotImplementedException();
		}

		public bool IsEmpty()
		{
			throw new NotImplementedException();
		}

		public int Lower()
		{
			throw new NotImplementedException();
		}

		public int Upper()
		{
			throw new NotImplementedException();
		}

		public bool IsDeletable()
		{
			throw new NotImplementedException();
		}

		public bool IsAllocated()
		{
			throw new NotImplementedException();
		}

						public Geom_Surface First()
		{
			throw new NotImplementedException();
		}

		public Geom_Surface ChangeFirst()
		{
			throw new NotImplementedException();
		}

		public Geom_Surface Last()
		{
			throw new NotImplementedException();
		}

		public Geom_Surface ChangeLast()
		{
			throw new NotImplementedException();
		}

		public Geom_Surface Value(int theIndex)
		{
			throw new NotImplementedException();
		}

		public Geom_Surface ChangeValue(int theIndex)
		{
			throw new NotImplementedException();
		}

		public void SetValue(int theIndex, Geom_Surface theItem)
		{
			throw new NotImplementedException();
		}

		public void Resize(int theLower, int theUpper, bool theToCopyData)
		{
			throw new NotImplementedException();
		}


	}; // class TColGeom_HArray1OfSurface

}
