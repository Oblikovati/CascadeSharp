//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKBRep.TopoDS;

namespace CascadeSharp.TKBool.TopOpeBRepBuild
{
    //---------------------------------------------------------------------
    //  Class  TopOpeBRepBuild_PaveSet
    //---------------------------------------------------------------------
    public sealed class TopOpeBRepBuild_PaveSet : TopOpeBRepBuild_LoopSet
    {
        public TopOpeBRepBuild_PaveSet(TopoDS_Shape E)
            : base()
        {
            throw new NotImplementedException();
        }

        public TopOpeBRepBuild_PaveSet(TopOpeBRepBuild_PaveSet parameter1)
            : base()
        {
            throw new NotImplementedException();
        }

        public TopOpeBRepBuild_PaveSet()
        {
            throw new NotImplementedException();
        }

        public void RemovePV(bool B)
        {
            throw new NotImplementedException();
        }

        public void Append(TopOpeBRepBuild_Pave PV)
        {
            throw new NotImplementedException();
        }

        public void InitLoop()
        {
            throw new NotImplementedException();
        }

        public bool MoreLoop()
        {
            throw new NotImplementedException();
        }

        public void NextLoop()
        {
            throw new NotImplementedException();
        }

        public TopOpeBRepBuild_Loop Loop()
        {
            throw new NotImplementedException();
        }

        public TopoDS_Edge Edge()
        {
            throw new NotImplementedException();
        }

        public bool HasEqualParameters()
        {
            throw new NotImplementedException();
        }

        public double EqualParameters()
        {
            throw new NotImplementedException();
        }

        public bool ClosedVertices()
        {
            throw new NotImplementedException();
        }

        public void SortPave(TopOpeBRepBuild_ListOfPave Lin, TopOpeBRepBuild_ListOfPave Lout)
        {
            throw new NotImplementedException();
        }

        public void Prepare()
        {
            throw new NotImplementedException();
        }
    }; // class TopOpeBRepBuild_PaveSet
}