//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;

namespace CascadeSharp.TKernel.TColStd
{
    //---------------------------------------------------------------------
    //  Class  TColStd_HArray1OfInteger
    //---------------------------------------------------------------------
    public sealed class TColStd_HArray1OfInteger : Standard_Transient
    {
        public TColStd_HArray1OfInteger()
            : base()
        {
            throw new NotImplementedException();
        }

        public TColStd_HArray1OfInteger(int theLower, int theUpper)
            : base()
        {
            throw new NotImplementedException();
        }

        public TColStd_HArray1OfInteger(int theLower, int theUpper, int theValue)
            : base()
        {
            throw new NotImplementedException();
        }

        public TColStd_HArray1OfInteger(TColStd_Array1OfInteger theOther)
            : base()
        {
            throw new NotImplementedException();
        }

        public TColStd_HArray1OfInteger(TColStd_HArray1OfInteger parameter1)
            : base()
        {
            throw new NotImplementedException();
        }

        public TColStd_Array1OfInteger Array1()
        {
            throw new NotImplementedException();
        }

        public TColStd_Array1OfInteger ChangeArray1()
        {
            throw new NotImplementedException();
        }

        public char get_type_name()
        {
            throw new NotImplementedException("Native class returns pointer to integer/double/handle.");
        }

        public void Init(int theValue)
        {
            throw new NotImplementedException();
        }

        public int Size()
        {
            throw new NotImplementedException();
        }

        public int Length()
        {
            throw new NotImplementedException();
        }

        public bool IsEmpty()
        {
            throw new NotImplementedException();
        }

        public int Lower()
        {
            throw new NotImplementedException();
        }

        public int Upper()
        {
            throw new NotImplementedException();
        }

        public bool IsDeletable()
        {
            throw new NotImplementedException();
        }

        public bool IsAllocated()
        {
            throw new NotImplementedException();
        }

        public int First()
        {
            throw new NotImplementedException();
        }

        public int ChangeFirst()
        {
            throw new NotImplementedException();
        }

        public int Last()
        {
            throw new NotImplementedException();
        }

        public int ChangeLast()
        {
            throw new NotImplementedException();
        }

        public int Value(int theIndex)
        {
            throw new NotImplementedException();
        }

        public int ChangeValue(int theIndex)
        {
            throw new NotImplementedException();
        }

        public void SetValue(int theIndex, int theItem)
        {
            throw new NotImplementedException();
        }

        public void Resize(int theLower, int theUpper, bool theToCopyData)
        {
            throw new NotImplementedException();
        }
    }; // class TColStd_HArray1OfInteger
}