//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;

namespace CascadeSharp.TKernel.TColStd
{
	//---------------------------------------------------------------------
	//  Class  TColStd_HArray1OfReal
	//---------------------------------------------------------------------
	public  sealed class TColStd_HArray1OfReal : Standard_Transient
	{

		public TColStd_HArray1OfReal()
			: base()
		{
			throw new NotImplementedException();
		}

		public TColStd_HArray1OfReal(int theLower, int theUpper)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColStd_HArray1OfReal(int theLower, int theUpper, double theValue)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColStd_HArray1OfReal(TColStd_Array1OfReal theOther)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColStd_HArray1OfReal(TColStd_HArray1OfReal parameter1)
			: base()
		{
			throw new NotImplementedException();
		}

		public TColStd_Array1OfReal Array1()
		{
			throw new NotImplementedException();
		}

		public TColStd_Array1OfReal ChangeArray1()
		{
			throw new NotImplementedException();
		}

		public char get_type_name()
		{
			throw new NotImplementedException("Native class returns pointer to integer/double/handle.");
		}

														public void Init(double theValue)
		{
			throw new NotImplementedException();
		}

		public int Size()
		{
			throw new NotImplementedException();
		}

		public int Length()
		{
			throw new NotImplementedException();
		}

		public bool IsEmpty()
		{
			throw new NotImplementedException();
		}

		public int Lower()
		{
			throw new NotImplementedException();
		}

		public int Upper()
		{
			throw new NotImplementedException();
		}

		public bool IsDeletable()
		{
			throw new NotImplementedException();
		}

		public bool IsAllocated()
		{
			throw new NotImplementedException();
		}

						public double First()
		{
			throw new NotImplementedException();
		}

		public double ChangeFirst()
		{
			throw new NotImplementedException();
		}

		public double Last()
		{
			throw new NotImplementedException();
		}

		public double ChangeLast()
		{
			throw new NotImplementedException();
		}

		public double Value(int theIndex)
		{
			throw new NotImplementedException();
		}

		public double ChangeValue(int theIndex)
		{
			throw new NotImplementedException();
		}

		public void SetValue(int theIndex, double theItem)
		{
			throw new NotImplementedException();
		}

		public void Resize(int theLower, int theUpper, bool theToCopyData)
		{
			throw new NotImplementedException();
		}


	}; // class TColStd_HArray1OfReal

}
