//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;

namespace CascadeSharp.TKernel.TShort
{
    //---------------------------------------------------------------------
    //  Class  TShort_HArray1OfShortReal
    //---------------------------------------------------------------------
    public sealed class TShort_HArray1OfShortReal : Standard_Transient
    {
        public TShort_HArray1OfShortReal()
            : base()
        {
            throw new NotImplementedException();
        }

        public TShort_HArray1OfShortReal(int theLower, int theUpper)
            : base()
        {
            throw new NotImplementedException();
        }

        public TShort_HArray1OfShortReal(int theLower, int theUpper, float theValue)
            : base()
        {
            throw new NotImplementedException();
        }

        public TShort_HArray1OfShortReal(TShort_Array1OfShortReal theOther)
            : base()
        {
            throw new NotImplementedException();
        }

        public TShort_HArray1OfShortReal(TShort_HArray1OfShortReal parameter1)
            : base()
        {
            throw new NotImplementedException();
        }

        public TShort_Array1OfShortReal Array1()
        {
            throw new NotImplementedException();
        }

        public TShort_Array1OfShortReal ChangeArray1()
        {
            throw new NotImplementedException();
        }

        public char get_type_name()
        {
            throw new NotImplementedException("Native class returns pointer to integer/double/handle.");
        }

        public void Init(float theValue)
        {
            throw new NotImplementedException();
        }

        public int Size()
        {
            throw new NotImplementedException();
        }

        public int Length()
        {
            throw new NotImplementedException();
        }

        public bool IsEmpty()
        {
            throw new NotImplementedException();
        }

        public int Lower()
        {
            throw new NotImplementedException();
        }

        public int Upper()
        {
            throw new NotImplementedException();
        }

        public bool IsDeletable()
        {
            throw new NotImplementedException();
        }

        public bool IsAllocated()
        {
            throw new NotImplementedException();
        }

        public float First()
        {
            throw new NotImplementedException();
        }

        public float ChangeFirst()
        {
            throw new NotImplementedException();
        }

        public float Last()
        {
            throw new NotImplementedException();
        }

        public float ChangeLast()
        {
            throw new NotImplementedException();
        }

        public float Value(int theIndex)
        {
            throw new NotImplementedException();
        }

        public float ChangeValue(int theIndex)
        {
            throw new NotImplementedException();
        }

        public void SetValue(int theIndex, float theItem)
        {
            throw new NotImplementedException();
        }

        public void Resize(int theLower, int theUpper, bool theToCopyData)
        {
            throw new NotImplementedException();
        }
    }; // class TShort_HArray1OfShortReal
}