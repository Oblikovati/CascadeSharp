//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------
using System;
using System.Collections.Generic;

namespace CascadeSharp
{
	//---------------------------------------------------------------------
	//  Class  BRepBuilderAPI_MakePolygon
	//---------------------------------------------------------------------
	public  sealed class BRepBuilderAPI_MakePolygon : CascadeSharp.BRepBuilderAPI_MakeShape
	{

		public BRepBuilderAPI_MakePolygon()
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepBuilderAPI_MakePolygon(gp_Pnt P1, gp_Pnt P2)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepBuilderAPI_MakePolygon(gp_Pnt P1, gp_Pnt P2, gp_Pnt P3, bool Close)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepBuilderAPI_MakePolygon(gp_Pnt P1, gp_Pnt P2, gp_Pnt P3)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepBuilderAPI_MakePolygon(gp_Pnt P1, gp_Pnt P2, gp_Pnt P3, gp_Pnt P4, bool Close)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepBuilderAPI_MakePolygon(gp_Pnt P1, gp_Pnt P2, gp_Pnt P3, gp_Pnt P4)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepBuilderAPI_MakePolygon(TopoDS_Vertex V1, TopoDS_Vertex V2)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepBuilderAPI_MakePolygon(TopoDS_Vertex V1, TopoDS_Vertex V2, TopoDS_Vertex V3, bool Close)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepBuilderAPI_MakePolygon(TopoDS_Vertex V1, TopoDS_Vertex V2, TopoDS_Vertex V3)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepBuilderAPI_MakePolygon(TopoDS_Vertex V1, TopoDS_Vertex V2, TopoDS_Vertex V3, TopoDS_Vertex V4, bool Close)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepBuilderAPI_MakePolygon(TopoDS_Vertex V1, TopoDS_Vertex V2, TopoDS_Vertex V3, TopoDS_Vertex V4)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepBuilderAPI_MakePolygon(BRepBuilderAPI_MakePolygon parameter1)
			: base()
		{
			throw new NotImplementedException();
		}

		public void Add(gp_Pnt P)
		{
			throw new NotImplementedException();
		}

		public void Add(TopoDS_Vertex V)
		{
			throw new NotImplementedException();
		}

		public bool Added()
		{
			throw new NotImplementedException();
		}

		public void Close()
		{
			throw new NotImplementedException();
		}

		public TopoDS_Vertex FirstVertex()
		{
			throw new NotImplementedException();
		}

		public TopoDS_Vertex LastVertex()
		{
			throw new NotImplementedException();
		}

		public bool IsDone()
		{
			throw new NotImplementedException();
		}

		public TopoDS_Edge Edge()
		{
			throw new NotImplementedException();
		}

		public TopoDS_Wire Wire()
		{
			throw new NotImplementedException();
		}


	}; // class BRepBuilderAPI_MakePolygon

}
