//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKMath.gp;

namespace CascadeSharp.TKGeomAlgo.Plate
{
    //---------------------------------------------------------------------
    //  Class  Plate_FreeGtoCConstraint
    //---------------------------------------------------------------------
    public sealed class Plate_FreeGtoCConstraint
    {
        public Plate_FreeGtoCConstraint(gp_XY point2d, Plate_D1 D1S, Plate_D1 D1T, double IncrementalLoad,
            int orientation)
            : base()
        {
            throw new NotImplementedException();
        }

        public Plate_FreeGtoCConstraint(gp_XY point2d, Plate_D1 D1S, Plate_D1 D1T, double IncrementalLoad)
            : base()
        {
            throw new NotImplementedException();
        }

        public Plate_FreeGtoCConstraint(gp_XY point2d, Plate_D1 D1S, Plate_D1 D1T)
            : base()
        {
            throw new NotImplementedException();
        }

        public Plate_FreeGtoCConstraint(gp_XY point2d, Plate_D1 D1S, Plate_D1 D1T, Plate_D2 D2S, Plate_D2 D2T,
            double IncrementalLoad, int orientation)
            : base()
        {
            throw new NotImplementedException();
        }

        public Plate_FreeGtoCConstraint(gp_XY point2d, Plate_D1 D1S, Plate_D1 D1T, Plate_D2 D2S, Plate_D2 D2T,
            double IncrementalLoad)
            : base()
        {
            throw new NotImplementedException();
        }

        public Plate_FreeGtoCConstraint(gp_XY point2d, Plate_D1 D1S, Plate_D1 D1T, Plate_D2 D2S, Plate_D2 D2T)
            : base()
        {
            throw new NotImplementedException();
        }

        public Plate_FreeGtoCConstraint(gp_XY point2d, Plate_D1 D1S, Plate_D1 D1T, Plate_D2 D2S, Plate_D2 D2T,
            Plate_D3 D3S, Plate_D3 D3T, double IncrementalLoad, int orientation)
            : base()
        {
            throw new NotImplementedException();
        }

        public Plate_FreeGtoCConstraint(gp_XY point2d, Plate_D1 D1S, Plate_D1 D1T, Plate_D2 D2S, Plate_D2 D2T,
            Plate_D3 D3S, Plate_D3 D3T, double IncrementalLoad)
            : base()
        {
            throw new NotImplementedException();
        }

        public Plate_FreeGtoCConstraint(gp_XY point2d, Plate_D1 D1S, Plate_D1 D1T, Plate_D2 D2S, Plate_D2 D2T,
            Plate_D3 D3S, Plate_D3 D3T)
            : base()
        {
            throw new NotImplementedException();
        }

        public Plate_FreeGtoCConstraint(Plate_FreeGtoCConstraint parameter1)
            : base()
        {
            throw new NotImplementedException();
        }

        public Plate_FreeGtoCConstraint()
        {
            throw new NotImplementedException();
        }

        public int nb_PPC()
        {
            throw new NotImplementedException();
        }

        public Plate_PinpointConstraint GetPPC(int Index)
        {
            throw new NotImplementedException();
        }

        public int nb_LSC()
        {
            throw new NotImplementedException();
        }

        public Plate_LinearScalarConstraint LSC(int Index)
        {
            throw new NotImplementedException();
        }
    }; // class Plate_FreeGtoCConstraint
}