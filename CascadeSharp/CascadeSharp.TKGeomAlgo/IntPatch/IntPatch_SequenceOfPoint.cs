//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKernel.NCollection;

namespace CascadeSharp.TKGeomAlgo.IntPatch
{
    //---------------------------------------------------------------------
    //  Class  IntPatch_SequenceOfPoint
    //---------------------------------------------------------------------
    public sealed class IntPatch_SequenceOfPoint
    {
        public IntPatch_SequenceOfPoint()
            : base()
        {
            throw new NotImplementedException();
        }

        public IntPatch_SequenceOfPoint(NCollection_BaseAllocator theAllocator)
            : base()
        {
            throw new NotImplementedException();
        }

        public IntPatch_SequenceOfPoint(IntPatch_SequenceOfPoint theOther)
            : base()
        {
            throw new NotImplementedException();
        }

        public int Size()
        {
            throw new NotImplementedException();
        }

        public int Length()
        {
            throw new NotImplementedException();
        }

        public int Lower()
        {
            throw new NotImplementedException();
        }

        public int Upper()
        {
            throw new NotImplementedException();
        }

        public bool IsEmpty()
        {
            throw new NotImplementedException();
        }

        public void Reverse()
        {
            throw new NotImplementedException();
        }

        public void Exchange(int I, int J)
        {
            throw new NotImplementedException();
        }

        public void Clear(NCollection_BaseAllocator theAllocator)
        {
            throw new NotImplementedException();
        }

        public void Clear()
        {
            throw new NotImplementedException();
        }

        public IntPatch_SequenceOfPoint Assign(IntPatch_SequenceOfPoint theOther)
        {
            throw new NotImplementedException();
        }

        public void Remove(int theIndex)
        {
            throw new NotImplementedException();
        }

        public void Remove(int theFromIndex, int theToIndex)
        {
            throw new NotImplementedException();
        }

        public void Append(IntPatch_Point theItem)
        {
            throw new NotImplementedException();
        }

        public void Append(IntPatch_SequenceOfPoint theSeq)
        {
            throw new NotImplementedException();
        }

        public void Prepend(IntPatch_Point theItem)
        {
            throw new NotImplementedException();
        }

        public void Prepend(IntPatch_SequenceOfPoint theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertBefore(int theIndex, IntPatch_Point theItem)
        {
            throw new NotImplementedException();
        }

        public void InsertBefore(int theIndex, IntPatch_SequenceOfPoint theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertAfter(int theIndex, IntPatch_SequenceOfPoint theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertAfter(int theIndex, IntPatch_Point theItem)
        {
            throw new NotImplementedException();
        }

        public void Split(int theIndex, IntPatch_SequenceOfPoint theSeq)
        {
            throw new NotImplementedException();
        }

        public IntPatch_Point First()
        {
            throw new NotImplementedException();
        }

        public IntPatch_Point ChangeFirst()
        {
            throw new NotImplementedException();
        }

        public IntPatch_Point Last()
        {
            throw new NotImplementedException();
        }

        public IntPatch_Point ChangeLast()
        {
            throw new NotImplementedException();
        }

        public IntPatch_Point Value(int theIndex)
        {
            throw new NotImplementedException();
        }

        public IntPatch_Point ChangeValue(int theIndex)
        {
            throw new NotImplementedException();
        }

        public void SetValue(int theIndex, IntPatch_Point theItem)
        {
            throw new NotImplementedException();
        }
    }; // class IntPatch_SequenceOfPoint
}