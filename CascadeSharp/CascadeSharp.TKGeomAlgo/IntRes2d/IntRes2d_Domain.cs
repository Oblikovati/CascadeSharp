//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKMath.gp;

namespace CascadeSharp.TKGeomAlgo.IntRes2d
{
    //---------------------------------------------------------------------
    //  Class  IntRes2d_Domain
    //---------------------------------------------------------------------
    public sealed class IntRes2d_Domain
    {
        public IntRes2d_Domain()
            : base()
        {
            throw new NotImplementedException();
        }

        public IntRes2d_Domain(gp_Pnt2d Pnt1, double Par1, double Tol1, gp_Pnt2d Pnt2, double Par2, double Tol2)
            : base()
        {
            throw new NotImplementedException();
        }

        public IntRes2d_Domain(gp_Pnt2d Pnt, double Par, double Tol, bool First)
            : base()
        {
            throw new NotImplementedException();
        }

        public IntRes2d_Domain(IntRes2d_Domain parameter1)
            : base()
        {
            throw new NotImplementedException();
        }

        public void SetValues(gp_Pnt2d Pnt1, double Par1, double Tol1, gp_Pnt2d Pnt2, double Par2, double Tol2)
        {
            throw new NotImplementedException();
        }

        public void SetValues()
        {
            throw new NotImplementedException();
        }

        public void SetValues(gp_Pnt2d Pnt, double Par, double Tol, bool First)
        {
            throw new NotImplementedException();
        }

        public void SetEquivalentParameters(double zero, double period)
        {
            throw new NotImplementedException();
        }

        public bool HasFirstPoint()
        {
            throw new NotImplementedException();
        }

        public double FirstParameter()
        {
            throw new NotImplementedException();
        }

        public gp_Pnt2d FirstPoint()
        {
            throw new NotImplementedException();
        }

        public double FirstTolerance()
        {
            throw new NotImplementedException();
        }

        public bool HasLastPoint()
        {
            throw new NotImplementedException();
        }

        public double LastParameter()
        {
            throw new NotImplementedException();
        }

        public gp_Pnt2d LastPoint()
        {
            throw new NotImplementedException();
        }

        public double LastTolerance()
        {
            throw new NotImplementedException();
        }

        public bool IsClosed()
        {
            throw new NotImplementedException();
        }

        public void EquivalentParameters(ref double zero, ref double zeroplusperiod)
        {
            throw new NotImplementedException();
        }
    }; // class IntRes2d_Domain
}