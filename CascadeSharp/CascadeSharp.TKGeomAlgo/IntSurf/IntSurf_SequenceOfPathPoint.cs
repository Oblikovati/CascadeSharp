//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKernel.NCollection;

namespace CascadeSharp.TKGeomAlgo.IntSurf
{
    //---------------------------------------------------------------------
    //  Class  IntSurf_SequenceOfPathPoint
    //---------------------------------------------------------------------
    public sealed class IntSurf_SequenceOfPathPoint
    {
        public IntSurf_SequenceOfPathPoint()
            : base()
        {
            throw new NotImplementedException();
        }

        public IntSurf_SequenceOfPathPoint(NCollection_BaseAllocator theAllocator)
            : base()
        {
            throw new NotImplementedException();
        }

        public IntSurf_SequenceOfPathPoint(IntSurf_SequenceOfPathPoint theOther)
            : base()
        {
            throw new NotImplementedException();
        }

        public int Size()
        {
            throw new NotImplementedException();
        }

        public int Length()
        {
            throw new NotImplementedException();
        }

        public int Lower()
        {
            throw new NotImplementedException();
        }

        public int Upper()
        {
            throw new NotImplementedException();
        }

        public bool IsEmpty()
        {
            throw new NotImplementedException();
        }

        public void Reverse()
        {
            throw new NotImplementedException();
        }

        public void Exchange(int I, int J)
        {
            throw new NotImplementedException();
        }

        public void Clear(NCollection_BaseAllocator theAllocator)
        {
            throw new NotImplementedException();
        }

        public void Clear()
        {
            throw new NotImplementedException();
        }

        public IntSurf_SequenceOfPathPoint Assign(IntSurf_SequenceOfPathPoint theOther)
        {
            throw new NotImplementedException();
        }

        public void Remove(int theIndex)
        {
            throw new NotImplementedException();
        }

        public void Remove(int theFromIndex, int theToIndex)
        {
            throw new NotImplementedException();
        }

        public void Append(IntSurf_PathPoint theItem)
        {
            throw new NotImplementedException();
        }

        public void Append(IntSurf_SequenceOfPathPoint theSeq)
        {
            throw new NotImplementedException();
        }

        public void Prepend(IntSurf_PathPoint theItem)
        {
            throw new NotImplementedException();
        }

        public void Prepend(IntSurf_SequenceOfPathPoint theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertBefore(int theIndex, IntSurf_PathPoint theItem)
        {
            throw new NotImplementedException();
        }

        public void InsertBefore(int theIndex, IntSurf_SequenceOfPathPoint theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertAfter(int theIndex, IntSurf_SequenceOfPathPoint theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertAfter(int theIndex, IntSurf_PathPoint theItem)
        {
            throw new NotImplementedException();
        }

        public void Split(int theIndex, IntSurf_SequenceOfPathPoint theSeq)
        {
            throw new NotImplementedException();
        }

        public IntSurf_PathPoint First()
        {
            throw new NotImplementedException();
        }

        public IntSurf_PathPoint ChangeFirst()
        {
            throw new NotImplementedException();
        }

        public IntSurf_PathPoint Last()
        {
            throw new NotImplementedException();
        }

        public IntSurf_PathPoint ChangeLast()
        {
            throw new NotImplementedException();
        }

        public IntSurf_PathPoint Value(int theIndex)
        {
            throw new NotImplementedException();
        }

        public IntSurf_PathPoint ChangeValue(int theIndex)
        {
            throw new NotImplementedException();
        }

        public void SetValue(int theIndex, IntSurf_PathPoint theItem)
        {
            throw new NotImplementedException();
        }
    }; // class IntSurf_SequenceOfPathPoint
}