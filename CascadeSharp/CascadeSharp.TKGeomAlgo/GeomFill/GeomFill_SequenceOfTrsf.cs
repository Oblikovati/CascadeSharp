//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKernel.NCollection;
using CascadeSharp.TKMath.gp;

namespace CascadeSharp.TKGeomAlgo.GeomFill
{
    //---------------------------------------------------------------------
    //  Class  GeomFill_SequenceOfTrsf
    //---------------------------------------------------------------------
    public sealed class GeomFill_SequenceOfTrsf
    {
        public GeomFill_SequenceOfTrsf()
            : base()
        {
            throw new NotImplementedException();
        }

        public GeomFill_SequenceOfTrsf(NCollection_BaseAllocator theAllocator)
            : base()
        {
            throw new NotImplementedException();
        }

        public GeomFill_SequenceOfTrsf(GeomFill_SequenceOfTrsf theOther)
            : base()
        {
            throw new NotImplementedException();
        }

        public int Size()
        {
            throw new NotImplementedException();
        }

        public int Length()
        {
            throw new NotImplementedException();
        }

        public int Lower()
        {
            throw new NotImplementedException();
        }

        public int Upper()
        {
            throw new NotImplementedException();
        }

        public bool IsEmpty()
        {
            throw new NotImplementedException();
        }

        public void Reverse()
        {
            throw new NotImplementedException();
        }

        public void Exchange(int I, int J)
        {
            throw new NotImplementedException();
        }

        public void Clear(NCollection_BaseAllocator theAllocator)
        {
            throw new NotImplementedException();
        }

        public void Clear()
        {
            throw new NotImplementedException();
        }

        public GeomFill_SequenceOfTrsf Assign(GeomFill_SequenceOfTrsf theOther)
        {
            throw new NotImplementedException();
        }

        public void Remove(int theIndex)
        {
            throw new NotImplementedException();
        }

        public void Remove(int theFromIndex, int theToIndex)
        {
            throw new NotImplementedException();
        }

        public void Append(gp_Trsf theItem)
        {
            throw new NotImplementedException();
        }

        public void Append(GeomFill_SequenceOfTrsf theSeq)
        {
            throw new NotImplementedException();
        }

        public void Prepend(gp_Trsf theItem)
        {
            throw new NotImplementedException();
        }

        public void Prepend(GeomFill_SequenceOfTrsf theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertBefore(int theIndex, gp_Trsf theItem)
        {
            throw new NotImplementedException();
        }

        public void InsertBefore(int theIndex, GeomFill_SequenceOfTrsf theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertAfter(int theIndex, GeomFill_SequenceOfTrsf theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertAfter(int theIndex, gp_Trsf theItem)
        {
            throw new NotImplementedException();
        }

        public void Split(int theIndex, GeomFill_SequenceOfTrsf theSeq)
        {
            throw new NotImplementedException();
        }

        public gp_Trsf First()
        {
            throw new NotImplementedException();
        }

        public gp_Trsf ChangeFirst()
        {
            throw new NotImplementedException();
        }

        public gp_Trsf Last()
        {
            throw new NotImplementedException();
        }

        public gp_Trsf ChangeLast()
        {
            throw new NotImplementedException();
        }

        public gp_Trsf Value(int theIndex)
        {
            throw new NotImplementedException();
        }

        public gp_Trsf ChangeValue(int theIndex)
        {
            throw new NotImplementedException();
        }

        public void SetValue(int theIndex, gp_Trsf theItem)
        {
            throw new NotImplementedException();
        }
    }; // class GeomFill_SequenceOfTrsf
}