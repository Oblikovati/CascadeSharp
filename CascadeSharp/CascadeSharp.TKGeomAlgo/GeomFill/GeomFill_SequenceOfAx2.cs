//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKernel.NCollection;
using CascadeSharp.TKMath.gp;

namespace CascadeSharp.TKGeomAlgo.GeomFill
{
	//---------------------------------------------------------------------
	//  Class  GeomFill_SequenceOfAx2
	//---------------------------------------------------------------------
	public  sealed class GeomFill_SequenceOfAx2
	{

		public GeomFill_SequenceOfAx2()
			: base()
		{
			throw new NotImplementedException();
		}

		public GeomFill_SequenceOfAx2(NCollection_BaseAllocator theAllocator)
			: base()
		{
			throw new NotImplementedException();
		}

		public GeomFill_SequenceOfAx2(GeomFill_SequenceOfAx2 theOther)
			: base()
		{
			throw new NotImplementedException();
		}

										public int Size()
		{
			throw new NotImplementedException();
		}

		public int Length()
		{
			throw new NotImplementedException();
		}

		public int Lower()
		{
			throw new NotImplementedException();
		}

		public int Upper()
		{
			throw new NotImplementedException();
		}

		public bool IsEmpty()
		{
			throw new NotImplementedException();
		}

		public void Reverse()
		{
			throw new NotImplementedException();
		}

		public void Exchange(int I, int J)
		{
			throw new NotImplementedException();
		}

				public void Clear(NCollection_BaseAllocator theAllocator)
		{
			throw new NotImplementedException();
		}

		public void Clear()
		{
			throw new NotImplementedException();
		}

		public GeomFill_SequenceOfAx2 Assign(GeomFill_SequenceOfAx2 theOther)
		{
			throw new NotImplementedException();
		}

				public void Remove(int theIndex)
		{
			throw new NotImplementedException();
		}

		public void Remove(int theFromIndex, int theToIndex)
		{
			throw new NotImplementedException();
		}

		public void Append(gp_Ax2 theItem)
		{
			throw new NotImplementedException();
		}

		public void Append(GeomFill_SequenceOfAx2 theSeq)
		{
			throw new NotImplementedException();
		}

		public void Prepend(gp_Ax2 theItem)
		{
			throw new NotImplementedException();
		}

		public void Prepend(GeomFill_SequenceOfAx2 theSeq)
		{
			throw new NotImplementedException();
		}

		public void InsertBefore(int theIndex, gp_Ax2 theItem)
		{
			throw new NotImplementedException();
		}

		public void InsertBefore(int theIndex, GeomFill_SequenceOfAx2 theSeq)
		{
			throw new NotImplementedException();
		}

				public void InsertAfter(int theIndex, GeomFill_SequenceOfAx2 theSeq)
		{
			throw new NotImplementedException();
		}

		public void InsertAfter(int theIndex, gp_Ax2 theItem)
		{
			throw new NotImplementedException();
		}

		public void Split(int theIndex, GeomFill_SequenceOfAx2 theSeq)
		{
			throw new NotImplementedException();
		}

		public gp_Ax2 First()
		{
			throw new NotImplementedException();
		}

		public gp_Ax2 ChangeFirst()
		{
			throw new NotImplementedException();
		}

		public gp_Ax2 Last()
		{
			throw new NotImplementedException();
		}

		public gp_Ax2 ChangeLast()
		{
			throw new NotImplementedException();
		}

		public gp_Ax2 Value(int theIndex)
		{
			throw new NotImplementedException();
		}

		public gp_Ax2 ChangeValue(int theIndex)
		{
			throw new NotImplementedException();
		}

		public void SetValue(int theIndex, gp_Ax2 theItem)
		{
			throw new NotImplementedException();
		}

				
	}; // class GeomFill_SequenceOfAx2

}
