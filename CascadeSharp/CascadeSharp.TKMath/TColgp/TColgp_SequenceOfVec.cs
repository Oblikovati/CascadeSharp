//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKernel.NCollection;
using CascadeSharp.TKMath.gp;

namespace CascadeSharp.TKMath.TColgp
{
    //---------------------------------------------------------------------
    //  Class  TColgp_SequenceOfVec
    //---------------------------------------------------------------------
    public sealed class TColgp_SequenceOfVec
    {
        public TColgp_SequenceOfVec()
            : base()
        {
            throw new NotImplementedException();
        }

        public TColgp_SequenceOfVec(NCollection_BaseAllocator theAllocator)
            : base()
        {
            throw new NotImplementedException();
        }

        public TColgp_SequenceOfVec(TColgp_SequenceOfVec theOther)
            : base()
        {
            throw new NotImplementedException();
        }

        public int Size()
        {
            throw new NotImplementedException();
        }

        public int Length()
        {
            throw new NotImplementedException();
        }

        public int Lower()
        {
            throw new NotImplementedException();
        }

        public int Upper()
        {
            throw new NotImplementedException();
        }

        public bool IsEmpty()
        {
            throw new NotImplementedException();
        }

        public void Reverse()
        {
            throw new NotImplementedException();
        }

        public void Exchange(int I, int J)
        {
            throw new NotImplementedException();
        }

        public void Clear(NCollection_BaseAllocator theAllocator)
        {
            throw new NotImplementedException();
        }

        public void Clear()
        {
            throw new NotImplementedException();
        }

        public TColgp_SequenceOfVec Assign(TColgp_SequenceOfVec theOther)
        {
            throw new NotImplementedException();
        }

        public void Remove(int theIndex)
        {
            throw new NotImplementedException();
        }

        public void Remove(int theFromIndex, int theToIndex)
        {
            throw new NotImplementedException();
        }

        public void Append(gp_Vec theItem)
        {
            throw new NotImplementedException();
        }

        public void Append(TColgp_SequenceOfVec theSeq)
        {
            throw new NotImplementedException();
        }

        public void Prepend(gp_Vec theItem)
        {
            throw new NotImplementedException();
        }

        public void Prepend(TColgp_SequenceOfVec theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertBefore(int theIndex, gp_Vec theItem)
        {
            throw new NotImplementedException();
        }

        public void InsertBefore(int theIndex, TColgp_SequenceOfVec theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertAfter(int theIndex, TColgp_SequenceOfVec theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertAfter(int theIndex, gp_Vec theItem)
        {
            throw new NotImplementedException();
        }

        public void Split(int theIndex, TColgp_SequenceOfVec theSeq)
        {
            throw new NotImplementedException();
        }

        public gp_Vec First()
        {
            throw new NotImplementedException();
        }

        public gp_Vec ChangeFirst()
        {
            throw new NotImplementedException();
        }

        public gp_Vec Last()
        {
            throw new NotImplementedException();
        }

        public gp_Vec ChangeLast()
        {
            throw new NotImplementedException();
        }

        public gp_Vec Value(int theIndex)
        {
            throw new NotImplementedException();
        }

        public gp_Vec ChangeValue(int theIndex)
        {
            throw new NotImplementedException();
        }

        public void SetValue(int theIndex, gp_Vec theItem)
        {
            throw new NotImplementedException();
        }
    }; // class TColgp_SequenceOfVec
}