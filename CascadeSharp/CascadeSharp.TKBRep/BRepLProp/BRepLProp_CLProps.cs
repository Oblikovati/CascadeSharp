//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKBRep.BRepAdaptor;
using CascadeSharp.TKMath.gp;

namespace CascadeSharp.TKBRep.BRepLProp
{
	//---------------------------------------------------------------------
	//  Class  BRepLProp_CLProps
	//---------------------------------------------------------------------
	public  sealed class BRepLProp_CLProps
	{

		public BRepLProp_CLProps(BRepAdaptor_Curve C, int N, double Resolution)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepLProp_CLProps(BRepAdaptor_Curve C, double U, int N, double Resolution)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepLProp_CLProps(int N, double Resolution)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepLProp_CLProps(BRepLProp_CLProps parameter1)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepLProp_CLProps()
		{
			throw new NotImplementedException();
		}

		public void SetParameter(double U)
		{
			throw new NotImplementedException();
		}

		public void SetCurve(BRepAdaptor_Curve C)
		{
			throw new NotImplementedException();
		}

		public gp_Pnt Value()
		{
			throw new NotImplementedException();
		}

		public gp_Vec D1()
		{
			throw new NotImplementedException();
		}

		public gp_Vec D2()
		{
			throw new NotImplementedException();
		}

		public gp_Vec D3()
		{
			throw new NotImplementedException();
		}

		public bool IsTangentDefined()
		{
			throw new NotImplementedException();
		}

		public void Tangent(gp_Dir D)
		{
			throw new NotImplementedException();
		}

		public double Curvature()
		{
			throw new NotImplementedException();
		}

		public void Normal(gp_Dir N)
		{
			throw new NotImplementedException();
		}

		public void CentreOfCurvature(gp_Pnt P)
		{
			throw new NotImplementedException();
		}


	}; // class BRepLProp_CLProps

}
