//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKBRep.BRepAdaptor;
using CascadeSharp.TKMath.GeomAbs;

namespace CascadeSharp.TKBRep.BRepLProp
{
    //---------------------------------------------------------------------
    //  Class  BRepLProp
    //---------------------------------------------------------------------
    public sealed class BRepLProp
    {
        public BRepLProp()
            : base()
        {
            throw new NotImplementedException();
        }

        public BRepLProp(BRepLProp parameter1)
            : base()
        {
            throw new NotImplementedException();
        }

        public GeomAbs_Shape Continuity(BRepAdaptor_Curve C1, BRepAdaptor_Curve C2, double u1, double u2, double tl,
            double ta)
        {
            throw new NotImplementedException();
        }

        public GeomAbs_Shape Continuity(BRepAdaptor_Curve C1, BRepAdaptor_Curve C2, double u1, double u2)
        {
            throw new NotImplementedException();
        }
    }; // class BRepLProp
}