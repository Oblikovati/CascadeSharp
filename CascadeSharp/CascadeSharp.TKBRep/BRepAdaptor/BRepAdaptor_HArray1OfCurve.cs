//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------
using System;
using System.Collections.Generic;

namespace CascadeSharp
{
	//---------------------------------------------------------------------
	//  Class  BRepAdaptor_HArray1OfCurve
	//---------------------------------------------------------------------
	public  sealed class BRepAdaptor_HArray1OfCurve : Standard_Transient
	{

		public BRepAdaptor_HArray1OfCurve()
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepAdaptor_HArray1OfCurve(int theLower, int theUpper)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepAdaptor_HArray1OfCurve(int theLower, int theUpper, BRepAdaptor_Curve theValue)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepAdaptor_HArray1OfCurve(BRepAdaptor_Array1OfCurve theOther)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepAdaptor_HArray1OfCurve(BRepAdaptor_HArray1OfCurve parameter1)
			: base()
		{
			throw new NotImplementedException();
		}

		public BRepAdaptor_Array1OfCurve Array1()
		{
			throw new NotImplementedException();
		}

		public BRepAdaptor_Array1OfCurve ChangeArray1()
		{
			throw new NotImplementedException();
		}

		public char get_type_name()
		{
			throw new NotImplementedException("Native class returns pointer to integer/double/handle.");
		}

														public void Init(BRepAdaptor_Curve theValue)
		{
			throw new NotImplementedException();
		}

		public int Size()
		{
			throw new NotImplementedException();
		}

		public int Length()
		{
			throw new NotImplementedException();
		}

		public bool IsEmpty()
		{
			throw new NotImplementedException();
		}

		public int Lower()
		{
			throw new NotImplementedException();
		}

		public int Upper()
		{
			throw new NotImplementedException();
		}

		public bool IsDeletable()
		{
			throw new NotImplementedException();
		}

		public bool IsAllocated()
		{
			throw new NotImplementedException();
		}

						public BRepAdaptor_Curve First()
		{
			throw new NotImplementedException();
		}

		public BRepAdaptor_Curve ChangeFirst()
		{
			throw new NotImplementedException();
		}

		public BRepAdaptor_Curve Last()
		{
			throw new NotImplementedException();
		}

		public BRepAdaptor_Curve ChangeLast()
		{
			throw new NotImplementedException();
		}

		public BRepAdaptor_Curve Value(int theIndex)
		{
			throw new NotImplementedException();
		}

		public BRepAdaptor_Curve ChangeValue(int theIndex)
		{
			throw new NotImplementedException();
		}

		public void SetValue(int theIndex, BRepAdaptor_Curve theItem)
		{
			throw new NotImplementedException();
		}

		public void Resize(int theLower, int theUpper, bool theToCopyData)
		{
			throw new NotImplementedException();
		}


	}; // class BRepAdaptor_HArray1OfCurve

}
