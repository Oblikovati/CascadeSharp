//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKernel.NCollection;
using CascadeSharp.TKMath.gp;

namespace CascadeSharp.TKFeat.LocOpe
{
    //---------------------------------------------------------------------
    //  Class  LocOpe_SequenceOfLin
    //---------------------------------------------------------------------
    public sealed class LocOpe_SequenceOfLin
    {
        public LocOpe_SequenceOfLin()
            : base()
        {
            throw new NotImplementedException();
        }

        public LocOpe_SequenceOfLin(NCollection_BaseAllocator theAllocator)
            : base()
        {
            throw new NotImplementedException();
        }

        public LocOpe_SequenceOfLin(LocOpe_SequenceOfLin theOther)
            : base()
        {
            throw new NotImplementedException();
        }

        public int Size()
        {
            throw new NotImplementedException();
        }

        public int Length()
        {
            throw new NotImplementedException();
        }

        public int Lower()
        {
            throw new NotImplementedException();
        }

        public int Upper()
        {
            throw new NotImplementedException();
        }

        public bool IsEmpty()
        {
            throw new NotImplementedException();
        }

        public void Reverse()
        {
            throw new NotImplementedException();
        }

        public void Exchange(int I, int J)
        {
            throw new NotImplementedException();
        }

        public void Clear(NCollection_BaseAllocator theAllocator)
        {
            throw new NotImplementedException();
        }

        public void Clear()
        {
            throw new NotImplementedException();
        }

        public LocOpe_SequenceOfLin Assign(LocOpe_SequenceOfLin theOther)
        {
            throw new NotImplementedException();
        }

        public void Remove(int theIndex)
        {
            throw new NotImplementedException();
        }

        public void Remove(int theFromIndex, int theToIndex)
        {
            throw new NotImplementedException();
        }

        public void Append(gp_Lin theItem)
        {
            throw new NotImplementedException();
        }

        public void Append(LocOpe_SequenceOfLin theSeq)
        {
            throw new NotImplementedException();
        }

        public void Prepend(gp_Lin theItem)
        {
            throw new NotImplementedException();
        }

        public void Prepend(LocOpe_SequenceOfLin theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertBefore(int theIndex, gp_Lin theItem)
        {
            throw new NotImplementedException();
        }

        public void InsertBefore(int theIndex, LocOpe_SequenceOfLin theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertAfter(int theIndex, LocOpe_SequenceOfLin theSeq)
        {
            throw new NotImplementedException();
        }

        public void InsertAfter(int theIndex, gp_Lin theItem)
        {
            throw new NotImplementedException();
        }

        public void Split(int theIndex, LocOpe_SequenceOfLin theSeq)
        {
            throw new NotImplementedException();
        }

        public gp_Lin First()
        {
            throw new NotImplementedException();
        }

        public gp_Lin ChangeFirst()
        {
            throw new NotImplementedException();
        }

        public gp_Lin Last()
        {
            throw new NotImplementedException();
        }

        public gp_Lin ChangeLast()
        {
            throw new NotImplementedException();
        }

        public gp_Lin Value(int theIndex)
        {
            throw new NotImplementedException();
        }

        public gp_Lin ChangeValue(int theIndex)
        {
            throw new NotImplementedException();
        }

        public void SetValue(int theIndex, gp_Lin theItem)
        {
            throw new NotImplementedException();
        }
    }; // class LocOpe_SequenceOfLin
}