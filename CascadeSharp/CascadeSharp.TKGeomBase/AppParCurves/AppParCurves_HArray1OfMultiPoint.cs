//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKernel;

namespace CascadeSharp.TKGeomBase.AppParCurves
{
	//---------------------------------------------------------------------
	//  Class  AppParCurves_HArray1OfMultiPoint
	//---------------------------------------------------------------------
	public  sealed class AppParCurves_HArray1OfMultiPoint : Standard_Transient
	{

		public AppParCurves_HArray1OfMultiPoint()
			: base()
		{
			throw new NotImplementedException();
		}

		public AppParCurves_HArray1OfMultiPoint(int theLower, int theUpper)
			: base()
		{
			throw new NotImplementedException();
		}

		public AppParCurves_HArray1OfMultiPoint(int theLower, int theUpper, AppParCurves_MultiPoint theValue)
			: base()
		{
			throw new NotImplementedException();
		}

		public AppParCurves_HArray1OfMultiPoint(AppParCurves_Array1OfMultiPoint theOther)
			: base()
		{
			throw new NotImplementedException();
		}

		public AppParCurves_HArray1OfMultiPoint(AppParCurves_HArray1OfMultiPoint parameter1)
			: base()
		{
			throw new NotImplementedException();
		}

		public AppParCurves_Array1OfMultiPoint Array1()
		{
			throw new NotImplementedException();
		}

		public AppParCurves_Array1OfMultiPoint ChangeArray1()
		{
			throw new NotImplementedException();
		}

		public char get_type_name()
		{
			throw new NotImplementedException("Native class returns pointer to integer/double/handle.");
		}

														public void Init(AppParCurves_MultiPoint theValue)
		{
			throw new NotImplementedException();
		}

		public int Size()
		{
			throw new NotImplementedException();
		}

		public int Length()
		{
			throw new NotImplementedException();
		}

		public bool IsEmpty()
		{
			throw new NotImplementedException();
		}

		public int Lower()
		{
			throw new NotImplementedException();
		}

		public int Upper()
		{
			throw new NotImplementedException();
		}

		public bool IsDeletable()
		{
			throw new NotImplementedException();
		}

		public bool IsAllocated()
		{
			throw new NotImplementedException();
		}

						public AppParCurves_MultiPoint First()
		{
			throw new NotImplementedException();
		}

		public AppParCurves_MultiPoint ChangeFirst()
		{
			throw new NotImplementedException();
		}

		public AppParCurves_MultiPoint Last()
		{
			throw new NotImplementedException();
		}

		public AppParCurves_MultiPoint ChangeLast()
		{
			throw new NotImplementedException();
		}

		public AppParCurves_MultiPoint Value(int theIndex)
		{
			throw new NotImplementedException();
		}

		public AppParCurves_MultiPoint ChangeValue(int theIndex)
		{
			throw new NotImplementedException();
		}

		public void SetValue(int theIndex, AppParCurves_MultiPoint theItem)
		{
			throw new NotImplementedException();
		}

		public void Resize(int theLower, int theUpper, bool theToCopyData)
		{
			throw new NotImplementedException();
		}


	}; // class AppParCurves_HArray1OfMultiPoint

}
