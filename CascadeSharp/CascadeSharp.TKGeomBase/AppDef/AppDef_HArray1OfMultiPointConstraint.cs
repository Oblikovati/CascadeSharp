//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKernel;

namespace CascadeSharp.TKGeomBase.AppDef
{
	//---------------------------------------------------------------------
	//  Class  AppDef_HArray1OfMultiPointConstraint
	//---------------------------------------------------------------------
	public  sealed class AppDef_HArray1OfMultiPointConstraint : Standard_Transient
	{

		public AppDef_HArray1OfMultiPointConstraint()
			: base()
		{
			throw new NotImplementedException();
		}

		public AppDef_HArray1OfMultiPointConstraint(int theLower, int theUpper)
			: base()
		{
			throw new NotImplementedException();
		}

		public AppDef_HArray1OfMultiPointConstraint(int theLower, int theUpper, AppDef_MultiPointConstraint theValue)
			: base()
		{
			throw new NotImplementedException();
		}

		public AppDef_HArray1OfMultiPointConstraint(AppDef_Array1OfMultiPointConstraint theOther)
			: base()
		{
			throw new NotImplementedException();
		}

		public AppDef_HArray1OfMultiPointConstraint(AppDef_HArray1OfMultiPointConstraint parameter1)
			: base()
		{
			throw new NotImplementedException();
		}

		public AppDef_Array1OfMultiPointConstraint Array1()
		{
			throw new NotImplementedException();
		}

		public AppDef_Array1OfMultiPointConstraint ChangeArray1()
		{
			throw new NotImplementedException();
		}

		public char get_type_name()
		{
			throw new NotImplementedException("Native class returns pointer to integer/double/handle.");
		}

														public void Init(AppDef_MultiPointConstraint theValue)
		{
			throw new NotImplementedException();
		}

		public int Size()
		{
			throw new NotImplementedException();
		}

		public int Length()
		{
			throw new NotImplementedException();
		}

		public bool IsEmpty()
		{
			throw new NotImplementedException();
		}

		public int Lower()
		{
			throw new NotImplementedException();
		}

		public int Upper()
		{
			throw new NotImplementedException();
		}

		public bool IsDeletable()
		{
			throw new NotImplementedException();
		}

		public bool IsAllocated()
		{
			throw new NotImplementedException();
		}

						public AppDef_MultiPointConstraint First()
		{
			throw new NotImplementedException();
		}

		public AppDef_MultiPointConstraint ChangeFirst()
		{
			throw new NotImplementedException();
		}

		public AppDef_MultiPointConstraint Last()
		{
			throw new NotImplementedException();
		}

		public AppDef_MultiPointConstraint ChangeLast()
		{
			throw new NotImplementedException();
		}

		public AppDef_MultiPointConstraint Value(int theIndex)
		{
			throw new NotImplementedException();
		}

		public AppDef_MultiPointConstraint ChangeValue(int theIndex)
		{
			throw new NotImplementedException();
		}

		public void SetValue(int theIndex, AppDef_MultiPointConstraint theItem)
		{
			throw new NotImplementedException();
		}

		public void Resize(int theLower, int theUpper, bool theToCopyData)
		{
			throw new NotImplementedException();
		}


	}; // class AppDef_HArray1OfMultiPointConstraint

}
