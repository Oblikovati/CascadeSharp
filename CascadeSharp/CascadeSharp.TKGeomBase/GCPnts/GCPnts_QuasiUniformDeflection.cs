//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKG2d.Adaptor2d;
using CascadeSharp.TKG3d.Adaptor3d;
using CascadeSharp.TKMath.GeomAbs;
using CascadeSharp.TKMath.gp;

namespace CascadeSharp.TKGeomBase.GCPnts
{
    //---------------------------------------------------------------------
    //  Class  GCPnts_QuasiUniformDeflection
    //---------------------------------------------------------------------
    public sealed class GCPnts_QuasiUniformDeflection
    {
        public GCPnts_QuasiUniformDeflection()
            : base()
        {
            throw new NotImplementedException();
        }

        public GCPnts_QuasiUniformDeflection(Adaptor3d_Curve C, double Deflection, GeomAbs_Shape Continuity)
            : base()
        {
            throw new NotImplementedException();
        }

        public GCPnts_QuasiUniformDeflection(Adaptor3d_Curve C, double Deflection)
            : base()
        {
            throw new NotImplementedException();
        }

        public GCPnts_QuasiUniformDeflection(Adaptor2d_Curve2d C, double Deflection, GeomAbs_Shape Continuity)
            : base()
        {
            throw new NotImplementedException();
        }

        public GCPnts_QuasiUniformDeflection(Adaptor2d_Curve2d C, double Deflection)
            : base()
        {
            throw new NotImplementedException();
        }

        public GCPnts_QuasiUniformDeflection(Adaptor3d_Curve C, double Deflection, double U1, double U2,
            GeomAbs_Shape Continuity)
            : base()
        {
            throw new NotImplementedException();
        }

        public GCPnts_QuasiUniformDeflection(Adaptor3d_Curve C, double Deflection, double U1, double U2)
            : base()
        {
            throw new NotImplementedException();
        }

        public GCPnts_QuasiUniformDeflection(Adaptor2d_Curve2d C, double Deflection, double U1, double U2,
            GeomAbs_Shape Continuity)
            : base()
        {
            throw new NotImplementedException();
        }

        public GCPnts_QuasiUniformDeflection(Adaptor2d_Curve2d C, double Deflection, double U1, double U2)
            : base()
        {
            throw new NotImplementedException();
        }

        public GCPnts_QuasiUniformDeflection(GCPnts_QuasiUniformDeflection parameter1)
            : base()
        {
            throw new NotImplementedException();
        }

        public void Initialize(Adaptor3d_Curve C, double Deflection, GeomAbs_Shape Continuity)
        {
            throw new NotImplementedException();
        }

        public void Initialize(Adaptor3d_Curve C, double Deflection)
        {
            throw new NotImplementedException();
        }

        public void Initialize(Adaptor2d_Curve2d C, double Deflection, GeomAbs_Shape Continuity)
        {
            throw new NotImplementedException();
        }

        public void Initialize(Adaptor2d_Curve2d C, double Deflection)
        {
            throw new NotImplementedException();
        }

        public void Initialize(Adaptor3d_Curve C, double Deflection, double U1, double U2, GeomAbs_Shape Continuity)
        {
            throw new NotImplementedException();
        }

        public void Initialize(Adaptor3d_Curve C, double Deflection, double U1, double U2)
        {
            throw new NotImplementedException();
        }

        public void Initialize(Adaptor2d_Curve2d C, double Deflection, double U1, double U2, GeomAbs_Shape Continuity)
        {
            throw new NotImplementedException();
        }

        public void Initialize(Adaptor2d_Curve2d C, double Deflection, double U1, double U2)
        {
            throw new NotImplementedException();
        }

        public bool IsDone()
        {
            throw new NotImplementedException();
        }

        public int NbPoints()
        {
            throw new NotImplementedException();
        }

        public double Parameter(int Index)
        {
            throw new NotImplementedException();
        }

        public gp_Pnt Value(int Index)
        {
            throw new NotImplementedException();
        }

        public double Deflection()
        {
            throw new NotImplementedException();
        }
    }; // class GCPnts_QuasiUniformDeflection
}