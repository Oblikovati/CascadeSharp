//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKMath.gp;

namespace CascadeSharp.TKGeomBase.GeomLib
{
    //---------------------------------------------------------------------
    //  Class  GeomLib_Array1OfMat
    //---------------------------------------------------------------------
    public sealed class GeomLib_Array1OfMat
    {
        public GeomLib_Array1OfMat()
            : base()
        {
            throw new NotImplementedException();
        }

        public GeomLib_Array1OfMat(int theLower, int theUpper)
            : base()
        {
            throw new NotImplementedException();
        }

        public GeomLib_Array1OfMat(GeomLib_Array1OfMat theOther)
            : base()
        {
            throw new NotImplementedException();
        }

        public GeomLib_Array1OfMat(gp_Mat theBegin, int theLower, int theUpper)
            : base()
        {
            throw new NotImplementedException();
        }

        public void Init(gp_Mat theValue)
        {
            throw new NotImplementedException();
        }

        public int Size()
        {
            throw new NotImplementedException();
        }

        public int Length()
        {
            throw new NotImplementedException();
        }

        public bool IsEmpty()
        {
            throw new NotImplementedException();
        }

        public int Lower()
        {
            throw new NotImplementedException();
        }

        public int Upper()
        {
            throw new NotImplementedException();
        }

        public bool IsDeletable()
        {
            throw new NotImplementedException();
        }

        public bool IsAllocated()
        {
            throw new NotImplementedException();
        }

        public GeomLib_Array1OfMat Assign(GeomLib_Array1OfMat theOther)
        {
            throw new NotImplementedException();
        }

        public GeomLib_Array1OfMat Move(GeomLib_Array1OfMat theOther)
        {
            throw new NotImplementedException();
        }

        public gp_Mat First()
        {
            throw new NotImplementedException();
        }

        public gp_Mat ChangeFirst()
        {
            throw new NotImplementedException();
        }

        public gp_Mat Last()
        {
            throw new NotImplementedException();
        }

        public gp_Mat ChangeLast()
        {
            throw new NotImplementedException();
        }

        public gp_Mat Value(int theIndex)
        {
            throw new NotImplementedException();
        }

        public gp_Mat ChangeValue(int theIndex)
        {
            throw new NotImplementedException();
        }

        public void SetValue(int theIndex, gp_Mat theItem)
        {
            throw new NotImplementedException();
        }

        public void Resize(int theLower, int theUpper, bool theToCopyData)
        {
            throw new NotImplementedException();
        }
    }; // class GeomLib_Array1OfMat
}