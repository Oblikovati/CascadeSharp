//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKernel.NCollection;

namespace CascadeSharp.TKGeomBase.FEmTool
{
	//---------------------------------------------------------------------
	//  Class  FEmTool_SeqOfLinConstr
	//---------------------------------------------------------------------
	public  sealed class FEmTool_SeqOfLinConstr
	{

		public FEmTool_SeqOfLinConstr()
			: base()
		{
			throw new NotImplementedException();
		}

		public FEmTool_SeqOfLinConstr(NCollection_BaseAllocator theAllocator)
			: base()
		{
			throw new NotImplementedException();
		}

		public FEmTool_SeqOfLinConstr(FEmTool_SeqOfLinConstr theOther)
			: base()
		{
			throw new NotImplementedException();
		}

										public int Size()
		{
			throw new NotImplementedException();
		}

		public int Length()
		{
			throw new NotImplementedException();
		}

		public int Lower()
		{
			throw new NotImplementedException();
		}

		public int Upper()
		{
			throw new NotImplementedException();
		}

		public bool IsEmpty()
		{
			throw new NotImplementedException();
		}

		public void Reverse()
		{
			throw new NotImplementedException();
		}

		public void Exchange(int I, int J)
		{
			throw new NotImplementedException();
		}

				public void Clear(NCollection_BaseAllocator theAllocator)
		{
			throw new NotImplementedException();
		}

		public void Clear()
		{
			throw new NotImplementedException();
		}

		public FEmTool_SeqOfLinConstr Assign(FEmTool_SeqOfLinConstr theOther)
		{
			throw new NotImplementedException();
		}

				public void Remove(int theIndex)
		{
			throw new NotImplementedException();
		}

		public void Remove(int theFromIndex, int theToIndex)
		{
			throw new NotImplementedException();
		}

				public void Append(FEmTool_SeqOfLinConstr theSeq)
		{
			throw new NotImplementedException();
		}

				public void Prepend(FEmTool_SeqOfLinConstr theSeq)
		{
			throw new NotImplementedException();
		}

				public void InsertBefore(int theIndex, FEmTool_SeqOfLinConstr theSeq)
		{
			throw new NotImplementedException();
		}

				public void InsertAfter(int theIndex, FEmTool_SeqOfLinConstr theSeq)
		{
			throw new NotImplementedException();
		}

				public void Split(int theIndex, FEmTool_SeqOfLinConstr theSeq)
		{
			throw new NotImplementedException();
		}

																		
	}; // class FEmTool_SeqOfLinConstr

}
