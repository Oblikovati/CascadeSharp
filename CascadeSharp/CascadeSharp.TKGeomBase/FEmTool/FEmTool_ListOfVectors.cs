//---------------------------------------------------------------------
// CascadeSharp
// General Public License V2
//---------------------------------------------------------------------

using System;
using CascadeSharp.TKernel.NCollection;
using CascadeSharp.TKernel.TColStd;

namespace CascadeSharp.TKGeomBase.FEmTool
{
	//---------------------------------------------------------------------
	//  Class  FEmTool_ListOfVectors
	//---------------------------------------------------------------------
	public  sealed class FEmTool_ListOfVectors
	{

		public FEmTool_ListOfVectors()
			: base()
		{
			throw new NotImplementedException();
		}

		public FEmTool_ListOfVectors(NCollection_BaseAllocator theAllocator)
			: base()
		{
			throw new NotImplementedException();
		}

		public FEmTool_ListOfVectors(FEmTool_ListOfVectors theOther)
			: base()
		{
			throw new NotImplementedException();
		}

										public int Size()
		{
			throw new NotImplementedException();
		}

		public FEmTool_ListOfVectors Assign(FEmTool_ListOfVectors theOther)
		{
			throw new NotImplementedException();
		}

		public void Clear(NCollection_BaseAllocator theAllocator)
		{
			throw new NotImplementedException();
		}

		public void Clear()
		{
			throw new NotImplementedException();
		}

		public TColStd_HArray1OfReal First()
		{
			throw new NotImplementedException();
		}

		public TColStd_HArray1OfReal Last()
		{
			throw new NotImplementedException();
		}

		public TColStd_HArray1OfReal Append(TColStd_HArray1OfReal theItem)
		{
			throw new NotImplementedException();
		}

				public void Append(FEmTool_ListOfVectors theOther)
		{
			throw new NotImplementedException();
		}

		public TColStd_HArray1OfReal Prepend(TColStd_HArray1OfReal theItem)
		{
			throw new NotImplementedException();
		}

		public void Prepend(FEmTool_ListOfVectors theOther)
		{
			throw new NotImplementedException();
		}

		public void RemoveFirst()
		{
			throw new NotImplementedException();
		}

												public void Reverse()
		{
			throw new NotImplementedException();
		}

				
	}; // class FEmTool_ListOfVectors

}
